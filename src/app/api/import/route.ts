    ...
    try {
        ...
        for (let i = startIndex; i < lines.length; i++) {
            ...
            try {
                ...
                // 3. Create Task (Check for duplicate legacy_id first)
                let taskId = uuidv4();
                let status = 'pending';
                if (rowData.status_str?.toLowerCase().includes('tamam')) status = 'completed';

                // Check if task exists by Legacy ID (SIRA)
                let skipInsert = false;
                if (rowData.legacy_id) {
                    const existRes = await connection.execute(
                        `SELECT id FROM tasks WHERE legacy_id = :1`,
                        [rowData.legacy_id]
                    );
                    if (existRes.rows && existRes.rows.length > 0) {
                        try {
                            taskId = (existRes.rows[0] as any)[0];
                            ...
                            skipInsert = true;
                            // Clean up existing assignees to re-add them
                            await connection.execute(
                ...
                if (!skipInsert) {
                    await connection.execute(
                        `INSERT INTO tasks (
                        "ID", "LIST_ID", "TITLE", "LEGACY_ID", "MEETING_DATE", "DUE_DATE", "NOTES", "CREATED_BY", "STATUS", "COMPLETED_AT"
                    ) VALUES (
                ...
                const handleAssignee = async (name: string, role: 'primary' | 'secondary') => {
                    ...
                    for (const personName of names) {
                        ...
                        if (userRes.rows && userRes.rows.length > 0) {
                            ...
                            userId = uuidv4();
                            // Generate email: adsoyad@optimed.com.tr
                            const emailPrefix = slugify(personName);
                            // Avoid empty emails
                            const placeholderEmail = (emailPrefix.length > 2 ? emailPrefix : `user${Date.now()}`) + '@optimed.com.tr';
                            ...
                            if (emailCheck.rows && emailCheck.rows.length > 0) {
                                userId = (emailCheck.rows[0] as any)[0];
                            } else {
                                await connection.execute(
