        ...
        <header className="h-14 border-b bg-background flex items-center justify-between px-6 shrink-0 relative z-10 transition-all">
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedListId && selectedList && !isFocusMode && !isMyTasksMode && (
              <TaskImportExport listId={selectedListId} listName={selectedList.name} />
            )}
          ...
          <div className="max-w-4xl mx-auto h-full overflow-visible">
            {viewMode === 'list' ? (
              <div className="space-y-8">
                {/* Görev ekleme: Sadece gerçek bir liste seçiliyken ve yetkisi varsa göster */}
                {(() => {
                  if (!selectedListId || selectedFolderId || isFocusMode) return null
                ...
                {/* Folder seçiliyken bilgi mesajı */}
                {(selectedFolderId && !isFocusMode) && (
                  <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-xl text-amber-700 dark:text-amber-400 text-sm flex items-center gap-3">
                    <div className="bg-amber-500/10 p-2 rounded-lg">📁</div>
                    <span>Görev eklemek için bu departmandaki bir <strong>liste</strong> seçin.</span>
                  </div>
                )}

                {/* Hiçbir şey seçilmemişken bilgi mesajı */}
                {(!selectedListId && !isFocusMode) && (
                  <div className="p-4 bg-primary/5 border border-primary/10 rounded-xl text-primary text-sm flex items-center gap-3">
                    ...
                    Görev eklemek için sol taraftan bir liste seçin.
                  </div>
                )}
